...
namespace BlobFileUpload.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult UploadChunk(int id)
        {
            ...
            {
                returnData = Json(new { error = true, isLastBLock = false, message = string.Format(CultureInfo.CurrentCulture, "Failed to Upload file", "Session Timed out.") });
                return returnData;
            }
        ...
        private JsonResult UploadCurrentChunk(CloudFile model, byte[] chunk, int id)
        {
            using (var chunkStream = new MemoryStream(chunk))
            {
                ...
                try
                {
                    model.BlockBlob.PutBlock(blockId, chunkStream, null, null, new BlobRequestOptions { RetryPolicy = new LinearRetry(TimeSpan.FromSeconds(10), 3) }, null);
                    return null;
                }
        ...
        private JsonResult CommitAllChunks(CloudFile model)
        {
            model.IsUploadCompleted = true;
            bool errorInOperation = false;
            try
            {
                var blockList = Enumerable.Range(1, (int)model.BlockCount)
                    .ToList<int>()
                    .ConvertAll(rangeElement =>
                        Convert.ToBase64String(Encoding.UTF8.GetBytes(string.Format(CultureInfo.InvariantCulture, "{0:D4}", rangeElement)))
                    );
